# enemy_combat_fsm.gd
class_name EnemyCombatFSM extends Node
...
var current_state: EnemyCombatState
...
func _create_states() -> void:
	...
	_add_state(State.IDLE, EnemyIdleState.new())
	_add_state(State.MOVE, EnemyMoveState.new())
	_add_state(State.ATTACK, EnemyAttackState.new())
	_add_state(State.STUN, EnemyStunState.new())

func _add_state(state_type: State, state_instance: EnemyCombatState) -> void:
