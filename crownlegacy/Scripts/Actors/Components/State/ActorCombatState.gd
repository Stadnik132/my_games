...
class_name ActorCombatState extends Node
...
var fsm: ActorCombatFSM      # Конечный автомат, которому принадлежит это состояние
var actor: Actor             # Актор, которым управляем
var brain: AIBrain           # Мозг, принимающий решения

func setup(p_fsm: ActorCombatFSM, p_actor: Actor, p_brain: AIBrain) -> void:
	fsm = p_fsm
	actor = p_actor
	brain = p_brain
