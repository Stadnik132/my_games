...
var _enemies_count_at_start: int = 0  # для расчёта опыта (при победе active_enemies уже пуст)
...
func _setup_connections() -> void:
	...
	#eb.Combat.enemy_damage_dealt.connect(_on_enemy_damage_dealt)
...
func end_combat(victory: bool, transition_to_dialogue: bool = false, dialogue_target: Node = null) -> void:
	...
	# Считаем награду ДО очистки списка врагов
	...
	if transition_to_dialogue and dialogue_target:
		# Переход в диалог (пощада и т.д.) — не считаем поражением
		var timeline_name = ""
		if dialogue_target is Actor:
			timeline_name = dialogue_target.dialogue_timeline
		if not timeline_name.is_empty():
			eb.Game.dialogue_requested.emit(timeline_name)
	elif not victory:
		# Если проиграли без перехода в диалог — Game Over
		eb.Game.game_over_requested.emit()
	else:
		# Победа — возврат в мир
		eb.Game.world_requested.emit()
...
func _handle_dialogue_choice(enemy: Node) -> void:
	...
	# Завершаем бой с переходом в диалог (диалог запросит end_combat)
...
func _on_enemy_damage_dealt(attacker: Node, target: Node, damage: int, damage_type: String):
	"""Обработчик: враг нанёс урон по цели"""
	if debug_mode:
		print_debug("CombatManager: получен урон от ", attacker.name, 
				   " по ", target.name, " (", damage, " ", damage_type, ")")
	
	# 1. Проверяем условия боя
	if is_decision_active:
		print_debug("  Урон заблокирован: активна точка решения")
		return
	
	# 2. ПРОВЕРЯЕМ триггеры (HP цели и т.д.)
	# TODO: добавить проверку триггеров для цели
	
	# 3. ПРОПУСКАЕМ урон дальше - к игроку
	if target.is_in_group("player"):
		# Отправляем запрос на получение урона
		EventBus.Player.damage_taken.emit(damage, damage_type, attacker, false, false)
	else:
		# Если цель не игрок (например, другой враг) - обрабатываем иначе
		push_warning("CombatManager: урон по не-игроку ", target.name)
...
func _calculate_experience(victory: bool) -> int:
	...
	if not victory:
		return 0
	# Используем сохранённое число врагов (при победе active_enemies уже очищен в _on_enemy_died)
	var total_exp = base_experience_per_enemy * (_enemies_count_at_start + 1)
